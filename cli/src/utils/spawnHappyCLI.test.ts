...
describe('spawnHappyCLI windowsHide behavior', () => {
  ...
  it('forces Bun child processes to run with the cli project root as cwd', async () => {
    ...
    const expectedProjectPath = projectPath().replace(/\\/g, '/');
    const expectedEntrypoint = `${expectedProjectPath}/src/index.ts`;

    expect(command.command).toBe(process.execPath);
    if (isBunRuntime) {
      expect(command.args[0]).toBe('--cwd');
      expect(command.args[1].replace(/\\/g, '/')).toBe(expectedProjectPath);
  ...
  it('passes the resolved compiled executable to child HAPI processes', async () => {
    ...
    const [command, , options] = spawnMock.mock.calls[0] as unknown[] | undefined ?? [];
