...
class KimiRemoteLauncher extends RemoteLauncherBase {
    ...
    protected async runMainLoop(): Promise<void> {
        ...
        if (!effectiveModel) {
            effectiveModel = backend.getSessionModelsMetadata(acpSessionId)?.currentModelId ?? null;
        }
    ...
    private async applyInitialModel(
        ...
        // Fall back to session/set_config_option with the fixed 'model' config
        // id — this backend does not expose a per-category config lookup, so
        // the id is assumed rather than discovered (mirrors the OpenCode
        // 'effort' config option convention in AcpSdkBackend).
        try {
            await backend.setConfigOption(sessionId, 'model', model);
            return model;
        } catch (error) {
            const message = error instanceof Error ? error.message : String(error);
