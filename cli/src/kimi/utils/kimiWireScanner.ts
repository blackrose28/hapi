...
/**
 * kimi-code's `step.end` usage summary does not map onto hapi's shared
 * `CodexMessage` union (which has no `token_count` variant on this fork —
 * usage tracking was never ported here, see acp-shared-fixes cart notes).
 * `session.sendAgentMessage` accepts `unknown`, so this local shape is
 * forwarded as-is without extending the shared CodexMessage union.
 */
export type KimiWireTokenCountMessage = {
    type: 'token_count';
    info: {
        total: {
            inputTokens: number;
...
export type KimiWireMessage = CodexMessage | KimiWireTokenCountMessage;
