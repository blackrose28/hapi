...
export function registerSessionConfigRpc<TPermissionMode extends PermissionMode>({
    ...
    rpcHandlerManager.registerHandler(RPC_METHODS.SetSessionConfig, async (payload: unknown) => {
        ...
        const config = payload as { permissionMode?: unknown; model?: unknown; effort?: unknown; modelReasoningEffort?: unknown }
        ...
        if (config.effort !== undefined) {
            ...
            if (effortMode === 'nullable') {
                next.effort = resolveNullableSessionModel(config.effort) // it does the exact same thing
        ...
        if (config.modelReasoningEffort !== undefined) {
            if (modelReasoningEffortMode === 'reject') {
                throw new Error('Invalid modelReasoningEffort')
            }
