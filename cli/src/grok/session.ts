...
export class GrokSession extends AgentSessionBase<GrokMode> {
    ...
    // Grok accepts a caller-supplied UUID as a brand-new native session id (via
    // `--session-id`), so local mode can seed a deterministic resume token
    // without scraping the fullscreen TUI. Track whether that id already
    // exists natively so the local launcher knows whether to pass
    // `--session-id` (create) or `--resume` (reattach).
