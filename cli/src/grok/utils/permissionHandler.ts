...
export class GrokPermissionHandler extends BasePermissionHandler<PermissionResponseMessage, void> {
    ...
    private handlePermissionRequest(request: PermissionRequest): void {
        ...
        // 'bypassPermissions' isn't handled generically by resolveAutoApprovalDecision
        // (that helper's rules are keyed off the master PermissionMode enum's
        // yolo/safe-yolo/read-only values), so it's special-cased here — mirroring
        // how Claude's own bypassPermissions handling short-circuits to
