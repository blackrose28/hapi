...
/**
 * Grok generates a session title through a separate auxiliary model route.
 * Accounts whose catalog only exposes the currently-active model can receive
 * a non-fatal 402 for that default grok-build request while the selected
 * model's turn continues normally. The prompt rejection, if any, is reported
 * separately by backend.prompt(), so suppressing this auxiliary stderr line
 * cannot hide a failed main turn.
 */
