...
// 2026-06-18) so the agent is no longer launchable. Keep an explicit
// tombstone command so `hapi gemini` reports a clear error instead of
// resolveCommand() falling through to the default Claude command with
// "gemini" forwarded as an argument (that silently started Claude).
