...
import type { PiThinkingLevel, PiModelSummary } from './types';
import type { ListSlashCommandsRequest, ListSlashCommandsResponse } from '@/modules/common/slashCommands';
...
export async function runPi(opts: {
    ...
    // codex-session-import-resume cart). Pi resume is handled entirely via
    // `bootstrapSession` + forwarding `--session-id` to the Pi subprocess below
    // (set from opts.resumeSessionId, itself populated by the runner via
    // buildCliArgs / the `pi` command's `--session-id`/`--resume` flag) —
    // matching the simpler bootstrapSession-only pattern already used by
    // runOpencode.ts/runKimi.ts.
    ...
    const { api, session: apiSession } = bootstrap;
    ...
    // --- Session config RPC ---
    //
    // Pi manually registers 'set-session-config' (the same wire method name
    // every other flavor's run*.ts registers) instead of a shared helper,
    // because Pi's wire protocol requires separate provider + modelId fields
    // (transport.send({ type: 'set_model', provider, modelId })), while a
    // generic string-model helper only handles model as a plain string. The
    // hub sends model as { provider, modelId } for Pi sessions specifically
    // (see hub/src/sync/sessionCache.ts#applySessionConfig).
    apiSession.rpcHandlerManager.registerHandler('set-session-config', async (rawPayload: unknown) => {
        ...
        // provider metadata (object check fails).
    ...
    // --- Slash commands (Pi native skills/commands) ---
    // Overrides the generic file-based listSlashCommands handler that
    // registerCommonHandlers() already installed on this session (RpcHandlerManager
    // uses last-registered-wins semantics), so Pi sessions surface Pi's own
    // native command/skill list instead of the generic .claude/commands-style scan.
    apiSession.rpcHandlerManager.registerHandler<ListSlashCommandsRequest, ListSlashCommandsResponse>(
        'listSlashCommands',
        async () => {
    ...
    apiSession.rpcHandlerManager.registerHandler('switch', async (payload: { to?: 'local' | 'remote' } = {}) => {
