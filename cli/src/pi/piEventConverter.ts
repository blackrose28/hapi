...
export function convertPiEvent(event: PiAgentEvent): AgentMessage[] {
    try {
        switch (event.type) {
            ...
            case 'turn_end': {
                // Note: our tree's AgentMessage/CodexMessage layer has no 'usage'
                // variant (usage-tracking was never ported into this fork's agent
                // message abstraction — see acp-shared-fixes cart's investigation
                // of AcpSdkBackend). Only emit turn_complete; token usage from
                // e.message?.usage is intentionally dropped rather than grafting
                // a parallel usage-reporting mechanism onto AgentMessage here.
