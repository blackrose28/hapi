...
// on the hub side; these runtime paths (get_state, startup set_model, successful
// set_model response) previously only keepAlive'd the bare modelId, so a Pi
// session on Pi's default model or started with --model could render/filter
...
function handleResponse(
    ...
    switch (command) {
        ...
        case 'set_model': {
            ...
            // set_model is awaited by SetSessionConfig; without this the awaited
            // RPC would time out and /sessions/:id/model return 409.
            resolvePendingRpc(resolver, response);
            break;
        }
        ...
        case 'get_available_models': {
            ...
            if (models.length > 0) {
                ...
                // blocked (it may be awaited by the pi-models REST endpoint).
