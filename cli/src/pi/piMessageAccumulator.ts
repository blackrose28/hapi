 ...
 * emit one reasoning + one text message per assistant message. Each
 * flush is a complete, final snapshot (not a partial delta), so unlike
 * upstream's version this does not need a streamId/id carried through
 * AgentMessage/CodexMessage for cross-message dedup — our tree's
 * `convertAgentMessage` always mints a fresh id per call and every
 * flush here already represents exactly one wire message per Pi
 * assistant message.
 */
