...
describe('PiMessageAccumulator', () => {
    ...
    it('accumulates thinking deltas and flushes one reasoning message on message_end', () => {
        ...
        const flushed = acc.handleEvent(makeEvent('message_end', { message: {} }));
        // NOTE: unlike upstream, our AgentMessage 'reasoning' variant has no
        // `id`/streamId field — convertAgentMessage always mints a fresh id
        // for the wire CodexMessage, and since this accumulator only ever
        // flushes one complete snapshot per Pi assistant message (not partial
        // deltas), there is nothing to dedup across messages that a carried-
        // through id would help with.
        expect(flushed).toEqual([
    ...
    it('accepts deltas regardless of contentIndex', () => {
        const acc = new PiMessageAccumulator();
        acc.handleEvent(makeEvent('message_start', { message: {} }));
        acc.handleEvent(makeEvent('message_update', {
    ...
    it('accumulates deltas across differing contentIndex values', () => {
        const acc = new PiMessageAccumulator();
        acc.handleEvent(makeEvent('message_start', { message: {} }));
        acc.handleEvent(makeEvent('message_update', {
