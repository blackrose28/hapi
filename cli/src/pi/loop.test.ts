...
describe('wireTransportEvents', () => {
    ...
    it('handles turn_start — pops pending localId', () => {
        ...
        expect(session.client.emitMessagesConsumed).toHaveBeenCalledWith(['id-turn-1']);
    });
    ...
    it('regression: agent_start + turn_start in one turn drains exactly one localId', () => {
        ...
        expect(session.client.emitMessagesConsumed).toHaveBeenCalledWith(['prompt-1']);
    });
...
//
// SetSessionConfig awaits set_model and set_thinking_level. A switch branch
// that updates state but never resolves the pending RPC would hit the 10s
// timeout and /sessions/:id/model would return 409 even though Pi accepted
// the change. These tests pin the contract: every awaited command must
// resolve before the timeout when Pi emits a success response.
...
describe('sendPiRpcAndWait', () => {
    ...
    it('set_model response resolves the awaited promise before timeout', async () => {
        ...
        // Must resolve (not reject with 'timed out').
    ...
    it('set_thinking_level response resolves the awaited promise before timeout', async () => {
        // Symmetry with set_model: set_thinking_level is awaited by
        // SetSessionConfig. Without an explicit resolve it fell to the
        // `default` branch; if anyone later adds business logic to a new
        // case without resolving first, the effort switch would time out
        // and /sessions/:id/effort would 409.
        const handlers = new Map<string, (...args: unknown[]) => void>();
        const { transport, reply } = recordingTransport(handlers);
        const session = createMockSession();
        wireTransportEvents(transport, session, []);
