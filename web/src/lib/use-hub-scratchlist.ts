...
import type { ScratchlistAttachmentMetadata } from '@hapi/protocol/schemas'
 ...
 * expect (`ScratchlistEntry` from `lib/scratchlist.ts`). Hub `id`
...
export function useHubScratchlist(
            ...
            try {
                ...
                for (const entry of localEntries) {
                    ...
                    try {
                        ...
                        // id so an idempotent retry doesn't land
    ...
    >({
        ...
        onMutate: async ({ text, attachments }) => {
            ...
            return { previousData, optimisticEntryId: optimistic.id }
        },
        onError: (_error, _variables, context) => {
            ...
            if (context?.optimisticEntryId) {
                queryClient.setQueryData<ScratchlistResponse>(queryKey, (prev) => ({
                    entries: (prev?.entries ?? []).filter((e) => e.id !== context.optimisticEntryId)
                }))
            }
        },
        onSuccess: (data, _variables, context) => {
            ...
            // id so we do not show duplicates client-side.
            queryClient.setQueryData<ScratchlistResponse>(queryKey, (prev) => {
                if (!prev) return { entries: [data.entry] }
                const without = prev.entries.filter((e) =>
                    e.id !== context?.optimisticEntryId
                    && e.id !== data.entry.id
                )
    ...
    >({
        mutationFn: async ({ id, text }) => {
            if (!api || !sessionId) throw new Error('Scratchlist unavailable')
            return await api.updateScratchlistEntry(sessionId, id, text)
        },
        ...
        onError: (error, _variables, context) => {
            if (isScratchlistNotFound(error)) {
                queryClient.setQueryData<ScratchlistResponse>(queryKey, (prev) => {
                    if (!prev) return prev
                    return { entries: prev.entries.filter((e) => e.id !== _variables.id) }
                })
    ...
    >({
        mutationFn: async ({ id }) => {
            if (!api || !sessionId) throw new Error('Scratchlist unavailable')
            await api.deleteScratchlistEntry(sessionId, id)
        },
        onMutate: async ({ id }) => {
            await queryClient.cancelQueries({ queryKey })
            const previousData = queryClient.getQueryData<ScratchlistResponse>(queryKey)
        ...
        onError: (error, variables, context) => {
            if (isScratchlistNotFound(error)) {
                queryClient.setQueryData<ScratchlistResponse>(queryKey, (prev) => {
                    if (!prev) return prev
                    return { entries: prev.entries.filter((e) => e.id !== variables.id) }
                })
                void queryClient.invalidateQueries({ queryKey })
                return
            }
    ...
    const updateEntry = useCallback(async (id: string, rawText: string) => {
        ...
        const truncated = text.length > SCRATCHLIST_MAX_TEXT_LENGTH
            ...
            : text
        try {
            await updateMutation.mutateAsync({ id: id, text: truncated })
        } catch {
    ...
    const move = useCallback((id: string, direction: 'up' | 'down') => {
        queryClient.setQueryData<ScratchlistResponse>(queryKey, (prev) => {
            ...
            // O(1) lookup.
            const byId = new Map(prev.entries.map((e) => [e.id, e] as const))
            const next: HubEntry[] = []
    ...
    useEffect(() => {
        ...
        if (!data) return
        try {
            const cached = (data as ScratchlistResponse).entries.map((e: HubEntry) => ({
                id: e.id,
                text: e.text,
    ...
    const entries: ScratchlistEntry[] = ((query.data as ScratchlistResponse | undefined)?.entries ?? []).map(toLocalEntry)
