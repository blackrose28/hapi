...
export function SessionExportDialog(props: SessionExportDialogProps) {
    ...
    return (
        <AppDialog open={props.isOpen} onOpenChange={(open) => !open && handleClose()}>
            <AppDialogContent presentation="sheet" className="max-w-sm">
                <AppDialogHeader title={t('session.export.title')} subtitle={t('session.export.description')} />
