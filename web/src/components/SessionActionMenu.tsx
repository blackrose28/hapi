    ...
    return createPortal(
        ...
        >
            ...
            >
                ...
                {(onOpenFiles || onExport || onUnpin || onSyncCodex) ? (
                    <div
                        className={`mx-2 my-0.5 h-px bg-[var(--app-divider)]${onOpenFiles && !filesVisibleOnDesktop && !onUnpin && !onExport && !onSyncCodex ? ' session-action-menu__mobile-only' : ''}`}
                        role="separator"
                    />
                ) : null}
                {sessionActive ? (
